...
func solution(_ diffs:[Int], _ times:[Int], _ limit:Int64) -> Int {
    ...
    while min < max {
        let mid = (max+min)/2
        print("시작 max: \(max), min: \(min), mid: \(mid)")
        let totalTime = totalTimes(level:mid, diffs:diffs, times:times)
        print("limit = \(limit), totalTime = \(totalTime)")
        ...
        print("끝 max: \(max), min: \(min)")
    }
    
    return min
}

func totalTimes(level:Int, diffs:[Int], times:[Int]) -> Int {
    var totalTime:Int = 0
    ...
    for i in diffs.indices {
        ...
        if diff <= level {
            ...
            totalTime += timeCur*(failCount+1) + timePrev*failCount
        }
    }
    return totalTime
}
